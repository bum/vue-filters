...
var filters = _objectSpread({}, _string__WEBPACK_IMPORTED_MODULE_8__); // Declare install function executed by Vue.use()
...
// CONCATENATED MODULE: ./src/filters/util.js
...
var capitalize_capitalize = function capitalize(val) {
  return isUndef(val) ? '' : val.toString().toLowerCase().replace(capitalizeRE, function (_, c) {
...
var lowercase_lowercase = function lowercase(val) {
  return isUndef(val) ? '' : val.toString().toLowerCase();
};
...
var placeholder_placeholder = function placeholder(val, _placeholder) {
  return isBlank(val) ? _placeholder : val;
};
...
var truncate_truncate = function truncate(val, length) {
  return isUndef(val) || !isString(val) || !isNumber(length) || length <= 3 || val.length <= length ? val : val.substring(0, length - 3) + '...';
};
...
var uppercase_lowercase = function lowercase(val) {
  return isUndef(val) ? '' : val.toString().toUpperCase();
};
...
/* concated harmony reexport lowercase */__webpack_require__.d(__webpack_exports__, "lowercase", function() { return /* unused export */ undefined; });
